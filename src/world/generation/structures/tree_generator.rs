...
use crate::world::generation::structures::structure_builder::StructureBuilder;

const CACTUS: BlockId = BlockId::Cactus;

fn make_cactus_1(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let mut builder = StructureBuilder::default();
    builder.make_column(x, z, y, rand.i32_in_range(4, 7), CACTUS);
    builder.build(chunk);
}

fn make_cactus_2(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let mut builder = StructureBuilder::default();
    let height = rand.i32_in_range(6, 8);
...
fn make_cactus_3(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let mut builder = StructureBuilder::default();
    let height = rand.i32_in_range(6, 8);
    builder.make_column(x, z, y, height, CACTUS);
    ...
    builder.make_row_x(z - 2, z + 2, x, stem + y, CACTUS);
...
pub fn make_oak_tree(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let mut builder = StructureBuilder::default();

    let h = rand.i32_in_range(4, 7);
    let leaf_size = 2;

    let new_y = h + y;
    builder.fill(
        new_y,
        x - leaf_size, x + leaf_size,
...
pub fn make_palm_tree(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let mut builder = StructureBuilder::default();

    let height = rand.i32_in_range(7, 9);
    let diameter = rand.i32_in_range(4, 6);
...
pub fn make_cactus(chunk: &mut Chunk, rand: &mut Random, x: i32, y: i32, z: i32) {
    let cac = rand.i32_in_range(0, 2);
    
    match cac {
        0 => make_cactus_1(chunk, rand, x, y, z),
        1 => make_cactus_2(chunk, rand, x, y, z),
        _ => make_cactus_3(chunk, rand, x, y, z)
    }
}