...
2026-05-01: F3 QA verdict set to REJECT because acceptance criteria explicitly requires no remaining `static mut` and no `p_` prefix identifiers, and both still occur in world generation code.
- 2026-05-01: Kept the noise generation behavior intact; the OnceLock only guards one-time initialization and still uses the existing `*SEED * 2` seed path.
